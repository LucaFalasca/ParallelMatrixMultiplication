...
def run_experiment_blocked(n_proc, pg_size, block_size, mat1_size, blocked_string, blocked_flag):
    command = "mpirun -n {} ./a.out {} {} {} data/matrix/bin/mat1_{}x{}.bin {} {} data/matrix/bin/mat2_{}x{}.bin {} {} data/matrix/bin/mat3_{}x{}.bin data/matrix/bin/mat3_{}x{}_check.bin data/out/result_{}.csv {}"
    for i in range(0, len(n_proc)):
        for j in range(0, len(mat1_size)):
            if blocked_flag==0:
                if (n_proc[i]==1 or n_proc[i]==2) and (mat1_size[j][0]==8192 and mat1_size[j][1]==8192):
                    continue
            curr_cmd=command.format(n_proc[i], pg_size[i][0], pg_size[i][1], block_size, mat1_size[j][0], mat1_size[j][1], mat1_size[j][0], mat1_size[j][1], mat1_size[j][0], mat1_size[j][1], mat1_size[j][0], mat1_size[j][1], mat1_size[j][0], mat1_size[j][1], mat1_size[j][0], mat1_size[j][1], blocked_string, blocked_flag)
            print("Launching ")
            print(curr_cmd)
            # Launch the C program using subprocess
            process = subprocess.Popen(curr_cmd, shell=True)
...
if __name__ == "__main__":
    ...
    run_experiment(n_proc, pg_size, block_size, mat1_size, mat2_size, mat3_size, 'blocked', 2)
